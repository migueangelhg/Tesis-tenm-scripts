...
library(furrr)
rm(list = ls())
tmin_paths <- list.files("~/Documentos/chelsa/tmin_1920_1980",pattern = ".tif$",
                         full.names = TRUE)
tmin_20_79 <- grep("_19[2-7][0-9]_V.1.0.tif",tmin_paths)
tmin_80 <- grep("_1980_V.1.0.tif",tmin_paths)

tmin_ids <- c(tmin_20_79,tmin_80)

tmin_paths_n <- tmin_paths[tmin_ids]
# nNA value -32768
...
x=1
if(!dir.exists("tmin_avg_1920_1980")) dir.create("tmin_avg_1920_1980")
months <- c(paste0("0",1:9),10:12)
filenames <- paste0("tmin_",months,".tif")
filepaths <- file.path("tmin_avg_1920_1980",filenames)
